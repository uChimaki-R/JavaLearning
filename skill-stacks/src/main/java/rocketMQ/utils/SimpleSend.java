...
public class SimpleSend {
    public static void send(String topic, String tag, String msg, int times) {
        DefaultMQProducer producer = new DefaultMQProducer("simple_send_producers");
        producer.setNamesrvAddr("192.168.202.135:9876");
        try {
            producer.start();
            System.out.println("SimpleSend 启动成功");
            // 异步发送且需要回调时可以使用计数器等待消息全部发送完毕
            CountDownLatch countDownLatch = new CountDownLatch(times);
            // 发送信息
            for (int i = 0; i < times; i++) {
                Message message = new Message(topic, tag, (msg + " to you the [" + i + "] times!").getBytes(StandardCharsets.UTF_8));
                ...
                producer.send(message, new SendCallback() {
                    ...
                    public void onSuccess(SendResult sendResult) {
                        synchronized (countDownLatch) {
                            ...
                            System.out.println("收到成功回复, 剩余未收到回复的消息数: " + countDownLatch.getCount());
                        }
                    }
                    ...
                    public void onException(Throwable throwable) {
                        synchronized (countDownLatch) {
                            countDownLatch.countDown();
                            System.out.println("<!> 收到失败回复, 剩余未收到回复的消息数: " + countDownLatch.getCount());
                        }
                    }
                });
